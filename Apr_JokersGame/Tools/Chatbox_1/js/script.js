...
function handleInput(event) {
    if (event.key === 'Enter') {
        ...
        if (result.length > 0) {
            ...
            simulateJokerTyping(answer, matched.audio);
            resetWrongQuestionCount();    //--wrong question counter
        } else {
            simulateJokerTyping(getWrongQuestionResponse());
            //----simulateJokerTyping("I don't understand your question.",null);
            trackWrongQuestion();    //--wrong question counter
        }
...
function simulateJokerTyping(reply, audioFile) {
    addMessage("...", 'joker');
    setTimeout(() => {
        ...
        // Play voice after typing
        if (audioFile) {
            const jokerVoice = new Audio('audio/' + audioFile);
            jokerVoice.play();
        }
